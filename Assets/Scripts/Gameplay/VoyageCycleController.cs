...
public class VoyageCycleController : MonoBehaviour
{
    ...
    [Header("Canvas UI")]
    [SerializeField] Button setSailButton;
    ...
    public void EditShip()
    {
        ...
        if (TryGetBoatRoot(out Transform boatRoot))
        {
            if (boatRoot.parent != null)
            {
                boatRoot.SetParent(null);
            }
            DontDestroyOnLoad(boatRoot.gameObject);
        }
    ...
    public void SetSail()
    {
        ...
        if (!HasBuiltBoat())
        {
            Debug.Log("Cannot set sail — no built boat found. Build a boat in the ship editor first.", this);
            return;
        }
    ...
    /// <summary>
    /// Called by ShipBuilder when saving a ship directly (bypassing SaveShip).
    /// Ensures the voyage state is Home so the sail scene initialises correctly.
    /// </summary>
    ...
    bool HasBuiltBoat()
    {
        return TryGetBoatRoot(out Transform boatRoot)
            && boatRoot.GetComponent<BoatMassManager>() != null;
    }
    ...
    void ApplyConfiguredBoatSpawnPose(Transform boatRoot)
    {
        ...
        {
            // No spawn point configured — place the boat at the world origin well above
            // the water surface so buoyancy can settle it correctly when physics is released.
            boatRoot.SetPositionAndRotation(Vector3.zero, Quaternion.identity);
        }
    ...
    void LockBoatMovement(Transform boatRoot)
    {
        ...
        if (!boatLockState.captured)
        {
            ...
            // If the boat arrives already fully frozen (e.g. freshly saved from the builder),
            // treat the restore target as None so SetSail actually frees it.
            RigidbodyConstraints currentConstraints = body != null ? body.constraints : RigidbodyConstraints.None;
            boatLockState.constraints = currentConstraints == RigidbodyConstraints.FreezeAll
    ...
    void RefreshBoundUi()
    {
        ...
        {
            setSailButton.interactable = HasBuiltBoat();
        }
